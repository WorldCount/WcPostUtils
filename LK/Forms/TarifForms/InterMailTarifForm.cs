...
namespace LK.Forms.TarifForms
{
    ...
    {
        private List<InterMailTarif> _tarifs;
        ...
        {
            _tarifs = await InterMailTarifManager.LoadAsync();
        ...
        private void dataGridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                InterMailTarif tarif = GetModelByRowIndex<InterMailTarif>(e.RowIndex);
        ...
        private void tbFilter_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                List<InterMailTarif> filtered = _tarifs.Where(m => m.Name.ToUpper().Contains(q) || m.Rate.ToString("N2").Contains(q) || m.RateNds.ToString("N2").Contains(q) || m.Mass.ToString().Contains(q) || m.TransType.ToString().ToUpper().Contains(q)).ToList();
        ...
        {
            _tarifs = await InterMailTarifManager.GetFromServer();
            UpdateData();
        ...
        {
            InterMailTarifManager.Save(_tarifs);
