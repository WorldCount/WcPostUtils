...
namespace LK.Forms.TarifForms
{
    ...
    public partial class TarificatorForm : Form
    {
        ...
        private void InitConfig(TarifType type)
        {
            ...
            _configNds = ConfigManager.GetConfigByName(ConfigName.Nds);
        }
        ...
        private void UpdateTarifs()
        {
            ...
            if (_tarifType == TarifType.Parcel)
                ParcelTarifs = Tarificator.ParcelTarificate(startCost, stepCost, startWeight, endWeight, step, _configNds.GetIntValue());

            if (_tarifType == TarifType.Mail)
                MailTarifs = Tarificator.MailTarificate(startCost, stepCost, startWeight, endWeight, step, _configNds.GetIntValue());
        }

        #region Обработчики

        private void TarificatorForm_Load(object sender, EventArgs e)
        {
