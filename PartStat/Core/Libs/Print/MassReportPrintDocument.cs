...
namespace PartStat.Core.Libs.Print
{
    public class MassReportPrintDocument : TablePrintDocument
    {
        ...
        // Кисти
        private SolidBrush ForeBrush { get; } = new SolidBrush(Color.Black);
        private Pen BorderBrush { get; } = Pens.Black;
        // Шрифты
        private Font BoldFont { get; } = new Font("Segoe Ui", 10, FontStyle.Bold);
        private Font DateFont { get; } = new Font("Segoe Ui", 9, FontStyle.Regular);
        private Font CellFont { get; } = new Font("Segoe Ui", 10, FontStyle.Regular);
        // Заголовок
        private Font HeaderFont { get; } = new Font("Segoe Ui", 10, FontStyle.Regular, GraphicsUnit.Point);
        private Font HeaderBoldFont { get; } = new Font("Segoe Ui", 12, FontStyle.Bold, GraphicsUnit.Point);
        ...
        private void PrintRow(PrintPageEventArgs e, DataGridViewRow gridRow, int topMargin)
        {
            ...
            while (cellCount < gridRow.Cells.Count)
            {
                ...
                if (cell?.Value != null)
                {
                    if (!clearRow && !clear)
                    {
                        e.Graphics.DrawString(cell.Value.ToString(), CellFont, ForeBrush, new Rectangle(colLeft, topMargin, colWidth, _cellHeight), _stringFormat);
                    }
                }
        ...
        private int PrintReportInfo(PrintPageEventArgs e, int margin, int pageWidth)
        {
            ...
            e.Graphics.DrawString(_singleReport.Name, HeaderBoldFont, ForeBrush, rect, stringFormat);
            ...
            e.Graphics.DrawString(info, HeaderFont, ForeBrush, rect, stringFormat);
            marginTop += offset;
        ...
        private int CalculateListInfo(PrintPageEventArgs e, DateList dateList)
        {
            ...
            size += (int) e.Graphics.MeasureString(dateList.Date.ToShortDateString(), BoldFont, area, stringFormat).Height;
            size += 20;

            area = new SizeF(colWidth, rowHeight);
            size += (int)e.Graphics.MeasureString(dateList.NumsToString(), BoldFont, area, stringFormat).Height;
        ...
        private int PrintListInfo(PrintPageEventArgs e, int margin, int newTopMargin, DateList dateList)
        {
            ...
            int rowHeight = _cellHeight + 20;
            e.Graphics.DrawString(dateList.Date.ToShortDateString(), BoldFont, ForeBrush, new Rectangle(colLeft, topMargin, colWidth, _cellHeight), stringFormat);
            topMargin += 20;

            e.Graphics.DrawString(info, CellFont, ForeBrush, new Rectangle(colLeft, topMargin, colWidth, rowHeight), stringFormat);
