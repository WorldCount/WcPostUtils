...
namespace PartStat.Core.Libs.Stats
{
    public class RpoStatCollector
    {
        ...
        public void Add(Rpo rpo)
        {
            ...
            // Счетчик ручки
            if (rpo.IsManual())
                ManualCount += rpo.Count;

            // Счетчик описей
            if (rpo.IsInventory())
                InventoryCount += rpo.Count;

            // Счетчик уведомлений
            if (rpo.IsInter())
            {
                if (rpo.IsSimpleNotice())
                    InterNoticeCount += rpo.Count;
            }
            else
            {
                if (rpo.IsSimpleNotice())
                    SimpleNoticeCount += rpo.Count;

                if (rpo.IsCustomNotice())
                    CustomNoticeCount += rpo.Count;
            }

            if (rpo.IsElectronicNotice())
                ElectronicNoticeCount += rpo.Count;

            if (data == null)
            {
                RpoStatData newData = new RpoStatData
                {
                    Count = rpo.Count, InterCode = rpo.IsInter(), MailCategory = rpo.MailCategory, MailType = rpo.MailType, Rate = rpo.MassRate
                };
        ...
        {
            return _statDatas.Where(s => s.MailType == 15).Where(s => s.MailCategory == 1).OrderBy(s => s.MassInt).ToArray();
        }
        ...
        {
            RpoStatData[] d = _statDatas.Where(s => s.MailType == 15).Where(s => s.MailCategory > 1).OrderBy(s => s.MassInt).ToArray();
            _statDatas = _statDatas.Except(d).ToList();
            return d;
        }
        ...
        {
            return _statDatas.Where(s => s.MailType == 16).Where(s => s.MailCategory == 1).OrderBy(s => s.TransType).ThenBy(s => s.MassInt).ToArray();
        }

        public RpoStatData[] GetFirstParcelStatExept()
        {
            RpoStatData[] d = _statDatas.Where(s => s.MailType == 16).Where(s => s.MailCategory == 1).OrderBy(s => s.TransType).ThenBy(s => s.MassInt).ToArray();
            _statDatas = _statDatas.Except(d).ToList();
            return d;
        }
        ...
        {
            return _statDatas.Where(s => s.MailType == 2).Where(s => s.MailCategory == 1).Where(s => s.InterCode == false).OrderBy(s => s.MassInt).ToArray();
        }

        public RpoStatData[] GetMailStatExcept()
        {
            RpoStatData[] d = _statDatas.Where(s => s.MailType == 2).Where(s => s.MailCategory == 1).Where(s => s.InterCode == false).OrderBy(s => s.MassInt).ToArray();
            _statDatas = _statDatas.Except(d).ToList();
            return d;
        }
        ...
        {
            return _statDatas.Where(s => s.MailType == 3).Where(s => s.MailCategory == 1).Where(s => s.InterCode == false).OrderBy(s => s.MassInt).ToArray();
        }

        public RpoStatData[] GetParcelStatExcept()
        {
            RpoStatData[] d = _statDatas.Where(s => s.MailType == 3).Where(s => s.MailCategory == 1).Where(s => s.InterCode == false).OrderBy(s => s.MassInt).ToArray();
            _statDatas = _statDatas.Except(d).ToList();
            return d;
        }
        ...
        {
            return _statDatas.Where(s => s.MailType == 2).Where(s => s.InterCode).Where(s => s.SimpleNotice == false).OrderBy(s => s.MassInt).ToArray();
        }

        public RpoStatData[] GetInterMailStatExcept()
        {
            RpoStatData[] d = _statDatas.Where(s => s.MailType == 2).Where(s => s.InterCode).Where(s => s.SimpleNotice == false).OrderBy(s => s.MassInt).ToArray();
            _statDatas = _statDatas.Except(d).ToList();
            return d;
        }
        ...
        {
            return _statDatas.Where(s => s.MailType == 2).Where(s => s.InterCode).Where(s => s.SimpleNotice).OrderBy(s => s.MassInt).ToArray();
        }

        public RpoStatData[] GetInterMailNoticeStatExcept()
        {
            RpoStatData[] d = _statDatas.Where(s => s.MailType == 2).Where(s => s.InterCode).Where(s => s.SimpleNotice).OrderBy(s => s.MassInt).ToArray();
            _statDatas = _statDatas.Except(d).ToList();
            return d;
        }
